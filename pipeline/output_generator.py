...
Fills family and environment fields left empty by the NKBA validator,
then serializes to disk and delegates rendering to render.py.
Rationale is now auto-generated by the validator from rule violations (no LLM call).
"""
...
class OutputGenerator:
    ...
    def __init__(
        ...
        """Initialise with output directory."""
    ...
    async def generate(
        ...
        Steps:
          ...
          3. Build structural layout items from input_json["environment"].
          4. Fill family / environment / layout on each variant (rationale already populated).
          5. Assemble FinalOutput.
          6. Write output.json.
          7. Run render.py subprocess (non-fatal on failure).
          8. Return FinalOutput.
        """
        ...
        # 3. Structural items (walls + openings)
        env = input_json.get("environment", {})
        structural_items = self._build_structural_items(env)

        # 4. Enrich each variant (rationale already populated by nkba_validator)
